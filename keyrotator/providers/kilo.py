...
async def call_with_pool(
    ...
    """
    Attempt Kilo generation using pool rotation.
    ...
    while True:
        ...
        if entry is None:
            raise AllKeysExhaustedError(
                f"All Kilo keys in pool are exhausted. "
        ...
        if entry.key in attempted_keys:
            ...
            raise AllKeysExhaustedError(
                f"All Kilo keys in pool are exhausted after attempting {len(attempted_keys)}. "
                "Check /dev/pool-status/ui for details."
            )
        ...
        try:
            logger.debug(f"Attempting Kilo call with key {entry.alias}")
            result = await _call_kilo_api(
                api_key=entry.key,
            ...
            if _is_quota_error(exc):
                logger.warning(
                    f"Kilo key {entry.alias} → {error_code} ({exc}). "
                ...
                logger.error(
                    f"Kilo key {entry.alias} → {error_code} ({exc}). Re-raising."
                )
                pool.report_error(entry, error_code)
                raise


async def _call_kilo_api(
    api_key: str,
    model_name: str,
    prompt: str,
    system_instruction: Optional[str],
    ...
    """
    Call Kilo Gateway API directly via REST.
    """
