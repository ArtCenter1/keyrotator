...
def KeyRotatorRouter(config_manager: ConfigManager) -> APIRouter:
    """Create and return a configured APIRouter for KeyRotator."""
    ...
    for provider_name, provider_config in config.providers.items():
        if provider_config.enabled:
            keys = config_manager.get_keys_for_provider(provider_name)
            if keys:  # Only create pool if there are keys
                pools[provider_name] = KeyPool(
    ...
    async def get_dashboard():
        ...
        status_data = [pool.get_status() for pool in pools.values()]
        ...
        status_json = json.dumps(
            {
                ...
                "is_development": config_manager.is_development_mode(),
            }
        )
    ...
    async def add_key(body: AddKeyRequest):
        ...
        # Add key to config
        config_manager.add_key(body.provider, body.key, body.alias)

        # Reload pools to include new key
        # Note: pools dict will be updated in the outer scope
    ...
    async def remove_key(provider: str, key_index: int):
        """Remove an API key."""
        success = config_manager.remove_key(provider, key_index)
        if not success:
    ...
    async def update_provider(provider: str, body: UpdateProviderRequest):
        ...
        config_manager.update_provider_config(provider, **update_data)
